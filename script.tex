...
\end{align}
A typical example of this is a two-body interaction $\Phi(x)=0$ if $|x|\neq 2$ \textcolor{red}{TODO: This has to be a set of different 'sites', but $x$ was used above to denote a single site, change hte notation}. Dynamics (in finite volume $\Lambda\in\mathcal{F}(\Gamma)$) are then given by \begin{align}
\tau_t^{\Lambda}(A)=e^{itH_{\Lambda}}Ae^{-itH_{\Lambda}}\; ,
\end{align}
which has a well defined limit as $\Lambda\rightarrow\Gamma$, i.e. there exist dynamics $\tau_t$ on $\mathcal{A}$ such that \begin{align}
\|\tau_t^{\Lambda}(A)-\tau_t(A)\|\rightarrow 0\; ,
\end{align}
as $\Lambda\rightarrow \Gamma$ under sufficient decay assumptions on $\|\phi(x)\|$ as diameter $\mathrm{diam}(x)$ \textcolor{red}{TODO: replace diam with $|x|$?} grows. The \underline{Mermin-Wagner theorem} now states that the absence of continuous symmetry breaking in $\nu=1,2$. Here consider the same Hamiltonian on all sites $\mathcal{H}_x=\mathcal{H}$ for all $x\in\Gamma$. And consider furthermore a continuous symmetry given by a compact connected Lie group $G$ (e.g. $SU(2)$). Assume there exists the following unitary representation of $G$ on $\mathcal{H}$ given by \begin{align}
U:G&\longrightarrow \mathcal{L}(\mathcal{H}) \\
g&\longmapsto U_g\; .
\end{align}
Whereby $U_gU_{g'}=U_{gg'}$, $U_{e}=\mathds{1}$ and $U_gU_g^*=U_g^*U_g=\mathds{1}$. Then this induces an action of $G$ on $\mathcal{A}_{G(x)}$ given by $A\mapsto U_g^*A U_g$. Furthermore, on $\mathcal{A}_{\Lambda}, \Lambda\in\mathcal{F}(\Gamma)$ by $\otimes_{x\in\Lambda}U_g$ and yields $*$-automorphisms of $\mathcal{A}$ \begin{align}
...
\end{align}
Note that $[E_m(x,y),O_m(x,y)]=0$ and $E_m(x,y)$ generates the same rotation at $x-$ and $y-$coordinates and \begin{align}\begin{split}
